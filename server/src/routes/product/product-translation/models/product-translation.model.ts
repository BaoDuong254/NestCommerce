...
export const GetProductTranslationParamsSchema = z
  .object({
    productTranslationId: z.coerce
      .number({ error: "Error.InvalidProductTranslationId" })
      .int({ error: "Error.ProductTranslationIdMustBeInteger" })
      .positive({ error: "Error.ProductTranslationIdMustBePositive" }),
  })
  .strict();
